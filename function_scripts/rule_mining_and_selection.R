...
select_measure_cutoffs <- function(selected_measures, df, resistance_indicator, target, data_source) {
  ...
  for (year in c(min(df[, "Year"]):max(df[, "Year"]))) {
    ...
    result <- tryCatch({
      ...
      ggsave(plot,
             filename = str_glue("{data_source}/figures/r_measure_dists/{resistance_indicator}/{data_source}_{year}_{resistance_indicator}_measure_distributions.png"))
...
rules_v_cutoffs <- function(df, resistance_indicator, target, measures, low, high, data_source) {
  ...
  for (measure in measures) {
    ...
    ggsave(plot = plot2, 
           filename = str_glue("{data_source}/figures/cut_off_selection/{resistance_indicator}/{data_source}_{resistance_indicator}_{target}_v_cutoffs_{measure}.png"), 
           height = 4, width = 6)
  }
}
...
save_top_gene_rules_aggregated_v2 <- function(Retail_Meats_df, cecal_df, NAHLN_df, target) {
  ...
  write.csv(full_df, "Unique_top_1000_rules_by_dataset.csv", row.names = FALSE)
...
save_top_gene_rules_aggregated_v3 <- function(Retail_Meats_df, cecal_df, NAHLN_df, target) {
  ...
  # Combine all datasets' top rules
  full_df <- bind_rows(dataset_rules)
  write.csv(full_df, "ClassDiverse_Unique_top_1000_rules_by_dataset.csv", row.names = FALSE)
