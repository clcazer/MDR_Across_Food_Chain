...
NAHLN_Class_level_phenotype_df <- read.csv("dataset_specific_outputs/NAHLN/NAHLN_wide_class_level_phenotype.csv")



NAHLN_genotype_df <- read.csv("dataset_specific_outputs/NAHLN/NAHLN_wide_corrected_genotype.csv")

NAHLN_Family_genotype_df <- read.csv("dataset_specific_outputs/NAHLN/NAHLN_wide_family_level_genotype.csv")

NAHLN_Class_level_genotype_df <- read.csv("dataset_specific_outputs/NAHLN/NAHLN_wide_class_level_genotype.csv")
...
#NETOWRK GRAPHS WERE NOT USED IN THE PAPER
...
# graph_rules(df = NAHLN_Class_level_genotype_df, target = "rules", cut_off = c(0.5, 0, 0.5, 0), resistance_indicator = "genotype", measures_used = c("cosine", "jaccard", "kulczynski", "support"), data_source = "NAHLN", rules_selected = "best", agg = TRUE)

print("DONE RUNNING THE NAHLN ANALYSIS")
