...
wrangle_NAHLN_phenotype <- function(df, break_points_df) {
    ...
    write.csv(resistance_df, "dataset_specific_outputs/NAHLN/NAHLN_wide_resStatus_phenotype.csv", row.names =FALSE)
...
keep_top_genes <- function(df, top_genes_df) {
    ...
    # Subset dataframe
    df <- df[, existing_cols]
    write.csv(df, "dataset_specific_outputs/NAHLN/NAHLN_top30_family_level_genotype.csv")
    return(df)
