...
plot_all_vs_best_rules <- function(df, resistance_indicator, target, cut_off, measures_used, data_source, class_level = FALSE) {
    ...
    if (length(year_list) > 0) {
        ...
        if (class_level) {
        write.csv(best_v_all_df, file = str_glue("dataset_specific_outputs/{data_source}/figures/pruning_results/{resistance_indicator}/{data_source}_{resistance_indicator}_Pruning_Results_CLASS.csv"), row.names = FALSE)
        } else {
        write.csv(best_v_all_df, file = str_glue("dataset_specific_outputs/{data_source}/figures/pruning_results/{resistance_indicator}/{data_source}_{resistance_indicator}_Pruning_Results.csv"), row.names = FALSE)
        }
        ...
        filename <- if (class_level) {
            str_glue("dataset_specific_outputs/{data_source}/figures/pruning_results/{resistance_indicator}/{data_source}_{resistance_indicator}_{target}_Rule_Pruning_Results_CLASS.png")
        } else {
            str_glue("dataset_specific_outputs/{data_source}/figures/pruning_results/{resistance_indicator}/{data_source}_{resistance_indicator}_{target}_Rule_Pruning_Results.png")
        }
