...
tabulate_and_compare <- function(df, rules_selected, cut_off, measures_used, resistance_indicator, data_source, class_level) {
   ...
   # df <- df[, names(df) != "MER"]
    ...
    write.csv(resistance_summary, str_glue("dataset_specific_outputs/{data_source}/tabulation_comp/{resistance_indicator}/{data_source}_{resistance_indicator}_tabulation{filename_suffix}.csv"), row.names = FALSE)
...
write.csv(combined_df, "combined_outputs/combined_csv_outputs/combined_tabComp_top20.csv", row.names = FALSE)
