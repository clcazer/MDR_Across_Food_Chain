...
get_virulence_gene_desciptives <- function(df, use_bayesian = FALSE) {
    ...
    for (source in data_sources) {
        ...
        write.csv(
            gene_prevalence,
            file = paste0("Virulence_Plasmid_outputs/virulence_descriptives/virulence_prevalence_", source, ".csv"),
            row.names = FALSE
        )
    ...
    if (use_bayesian) {
        ...
        write.csv(convergence_results,
                 "Virulence_Plasmid_outputs/virulence_descriptives/virulence_genes_bayesian_convergence.csv",
                 row.names = FALSE)
        ...
        write.csv(statistical_results,
            "Virulence_Plasmid_outputs/virulence_descriptives/virulence_genes_bayesian_main_test.csv",
            row.names = FALSE)
        ...
        if (length(levels(all_counts$Source)) > 1) {
            ...
            dir.create("Virulence_Plasmid_outputs/virulence_descriptives/posterior_plots", 
            ...
            for (i in 1:ncol(pairs)) {
                ...
                # Create density plot
                png(sprintf("Virulence_Plasmid_outputs/virulence_descriptives/posterior_plots/comparison_%s_vs_%s.png", 
                ...
                png(sprintf("Virulence_Plasmid_outputs/virulence_descriptives/posterior_plots/trace_%s_vs_%s.png", 
        ...
        write.csv(
            ...
            "Virulence_Plasmid_outputs/virulence_descriptives/virulence_genes_bayesian_estimates.csv",
            row.names = FALSE
        )
        ...
        if (!is.null(source_comparisons)) {
            write.csv(
                source_comparisons,
                "Virulence_Plasmid_outputs/virulence_descriptives/virulence_genes_bayesian_comparisons.csv",
                row.names = FALSE
            )
        }
        ...
        results$trace_plots <- "Saved in Virulence_Plasmid_outputs/virulence_descriptives/posterior_plots/"
        ...
        write.csv(statistical_results,
                 "Virulence_Plasmid_outputs/virulence_descriptives/virulence_genes_frequentist_main_test.csv",
                 row.names = FALSE)
        
        if (nrow(pairwise_comparisons) > 0) {
            write.csv(pairwise_comparisons,
                     "Virulence_Plasmid_outputs/virulence_descriptives/virulence_genes_frequentist_pairwise.csv",
                     row.names = FALSE)
        }
    ...
    write.csv(genes_per_isolate_stats, 
              "Virulence_Plasmid_outputs/virulence_descriptives/virulence_genes_per_isolate_stats.csv", 
              row.names = FALSE)
    
    return(results)
}

virulence_df <- read.csv("Raw_Data/Virulence_Plasmid_data/amrfinder_results-VIRULENCE-PLUS.point-muts-included.csv")
...
create_rarefaction_curves <- function(df, n_iterations = 100) {
    ...
    # Save data
    write.csv(all_results, "Virulence_Plasmid_outputs/virulence_descriptives/rarefaction_data.csv", row.names = FALSE)
