...
Retail_Meats_MIC_to_interval <- function(df, data_source, limit_date_range = FALSE) {
    ...
    write.csv(merged_df, file = str_glue("dataset_specific_outputs/{data_source}/NEW_{data_source}_phenotype_data.csv"), row.names = FALSE)
...
# df <- read.csv("Retail_Meats_Data/CVM-NARMS-Retail-Meats.csv")

# Retail_Meats_MIC_to_interval(df = df, data_source = "Retail_Meats")




Retail_Meats_process_genotype_data <- function(gene_df, ID_Year_lookup_df, data_source, output_level = 'gene', limit_date_range = FALSE) {
    gene_df <- gene_df[gene_df$meta == "NARMS_MEAT", ]
    print(length(unique(gene_df$SAMN)))
    print(length(gene_df$SAMN))
    
    # Create a mapping dataframe with just the columns we need
    id_year_mapping <- ID_Year_lookup_df[, c("BIOSAMPLE_ID", "ID", "Year")]
   ...
   if (output_level == "gene") {
    ...
    write.csv(incidence_matrix, file = str_glue("dataset_specific_outputs/{data_source}/{data_source}_wide_corrected_genotype.csv"), row.names = FALSE)
    
    } else if (output_level == "class") {
    
    write.csv(incidence_matrix, file = str_glue("dataset_specific_outputs/{data_source}/{data_source}_wide_class_level_genotype.csv"), row.names = FALSE)
    
    } else if (output_level == "subclass") {
    
    write.csv(incidence_matrix, file = str_glue("dataset_specific_outputs/{data_source}/{data_source}_wide_Subclass_level_genotype.csv"), row.names = FALSE)
...
# incidence_matrix <- Retail_Meats_process_genotype_data(gene_df, ID_Year_lookup_df, data_source = "Retail_Meats", output_level = "gene")
