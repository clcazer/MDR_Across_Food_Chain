...
graph_rules <- function(df, target, cut_off, resistance_indicator, measures_used, data_source, rules_selected, agg = FALSE){
    ...
    for (year in c(min(df$Year):max(df$Year))){
        ...
        if (rules_selected == "best"){
            rules <- implement_cuts(df = df, resistance_indicator = resistance_indicator, target = target, cut_off = cut_off, year = year, agg = agg, measures_used = measures_used)
        } else if (rules_selected == "all"){
            rules <- get_rules_or_itemsets(df = df, resistance_indicator = resistance_indicator, year = year, target = target, agg = agg)
        }
        ...
        # Add LHS class
        decomposed_rules$LHS_class <- sapply(decomposed_rules$LHS, function(x) {
            match_row <- which(genotype_class_mappings$Gene_famile == x)
            if (length(match_row) > 0) {
                return(genotype_class_mappings$sub_class[match_row[1]])
            } else {
                return(NA)
            }
        })
        ...
        # Set edge attributes
        E(rules_graph)$width <- ((decomposed_rules$count-min(decomposed_rules$count))/(max(decomposed_rules$count)-min(decomposed_rules$count)) + 0.1)* 10  # Edge thickness proportional to rule count (normalized)
        ...
        if (min_degree == max_degree) {
            V(rules_graph)$size <- 35  # Default size if all degrees are the same
        } else {
            V(rules_graph)$size <- 30 * ( 1 +(node_degrees - min_degree) / (max_degree - min_degree))
        }
...
# genotype_df = read.csv("Retail_Meats/wide_corrected_genotype.csv")
# genotype_df <- subset(genotype_df, select = -c(fosA7, mdf.A., mcr.9))
